...
export class PythonSetup {
  ...
  async findSystemPython(): Promise<string | null> {
  ...
  async createVenv(
    venvName: string = '.venv'
  ): Promise<{ success: boolean; pythonPath?: string; error?: string }> {
    // Find system Python to create venv with
    const systemPython = await this.findSystemPython();
    if (!systemPython) {
      return {
        success: false,
        error: 'No system Python found. Please install Python 3.8+ first.',
      };
    }
    ...
    if (await directoryExists(venvDir)) {
      return {
        success: false,
        error: `Directory "${venvName}" already exists.`,
      };
    }
      ...
      proc.on('close', async (code) => {
        if (code === 0) {
          ...
          if (pythonPath) {
            ...
            resolve({ success: false, error: 'Venv created but Python not found inside.' });
          }
        } else {
          resolve({ success: false, error: stderr || `venv creation failed with code ${code}` });
        }
      });
      proc.on('error', (err) => {
        resolve({ success: false, error: err.message });
